...
//! \fixme current state of the art is ryu
...
K G[26];V dis(K,I);ZK es();ZK K0;K k0(){R r1(K0);}ZK vf(I f){K r=kS(0);N(26,K x=G[i];if(NL-x)if(f^(Ax||xt))r=j2(r,ks(i)))R r;}
...
ZK1(se){
  ...
  x=sS(";      "[t],e1(se,x));
  R!t?cj('(',jc(x,')')):x;}
#define DIS 0 //!< \fixme DIS condition is too crude to detect (source;opcodes) tuple
