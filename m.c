...
K m1(J n){K x,r;I i=clzl(n+7),j;P((x=M[i]),M[i]=xx,x)j=i;
 W(!(x=++j<31?M[j]:8+ma(0,16L<<(j=MX(18,i)))));
 xm=i,M[j]=xx,r=x;W(i<j)x+=16L<<xm,M[*(J*)(x-8)=i++]=x,xx=0;R r;}
K1(l2){R kp("[]");}//!< struct/fixedarray experiment
V1(l0){if((J)xy)l0(xy),l0(xz);xx=M[xm],M[xm]=x;}K3(l1){K r=m1(24);R rt=8,rn=3,rx=x,ry=y,rz=z,r;}
