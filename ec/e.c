...
Z_ sel(S r,S x,S y,G c){X(Rg=-c&(Yg^Xg)^Xg)}//!< select, normalize, product, subtract, negate, inverse, xint, equal, sum
...
Z_ xadd(S x5,S z5,S x1,S z1,S x2,S z2,S x3,S z3){FP(da)FP(cb)FP(a)FP(b)
   ...
   sum(a,da,cb),sq(b,a),prd(x5,z1,b),sub(a,da,cb),sq(b,a),prd(z5,x1,b);}

// dbl-1987-m: x3=(x1^2-z1^2)^2, z3=4 x1 z1 (x1^2+(a x1 z1)+z1^2)
Z_ xdbl(S x3,S z3,S x1,S z1){FP(x1sq)FP(z1sq)FP(x1z1)FP(a)
   sq(x1sq,x1),sq(z1sq,z1),prd(x1z1,x1,z1),sub(a,x1sq,z1sq),sq(x3,a),
...
Z_ xmul(S r,S q,S e){FP(xm)FP(xms)FP(zms)G zm[32]={1},xm1[32]={1},zm1[32]={0};dsn(xm,q,32);// p(m), p(m-1)
   ...
   inv(zm1,zm),prd(r,zm1,xm),nrm(r);}// freeze out projective coords
