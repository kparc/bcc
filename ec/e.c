...
Z_ sel(S r,S x,S y,G c){X(Rg=-c&(Yg^Xg)^Xg)}//!< select, normalize, product, subtract, negate, inverse, timesconst, sum.
...
// x25519 ecdh
_ xkeygen(G sec[32],G pub[32],G rnd[32]){dsn(sec,rnd,32);prp(sec),xmul(pub,BP,sec);}
